...
export function ProfileRoute() {
  ...
  return (
    <>
          ...
          <div className="rigth">
            ...
            <Skeleton loading={isLoading}>
              <div className="info-section">
                <div className="text">
                  {data?.informations != null && data?.informations.split("\\n").map((line: string, index: number) => (
                    <span key={index}>
                      {line}
                      <br />
                    </span>
      ...
      <Skeleton loading={isLoading}>
        <div className="middle">{/* Admin controls commented out */}</div>
      </Skeleton>
      ...
      <div className={"bottom"}>
        ...
        {data?.category && (data?.category.categories?.length > 0 || data?.category.type?.length > 0) && (
          <Skeleton loading={isLoading}>
            <div className="categories-section">
              <div className="section-card">
                ...
                <div className="tags-container">
                  ...
                  {data?.category.categories?.map((cat: string, index: number) => (
            ...
            <div className="section-card">
              <div className="address-container">
                ...
                <div className="address-content">
                  ...
                  <p className="address-text">{formatAddress() || data?.resume || "Endereço não disponível"}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {(data?.telephones?.whatsapp?.length > 0 || data?.telephones?.telephone?.length > 0) && (
          <Skeleton loading={isLoading}>
            <div className="contact-section">
              <div className="section-card">
                <h3 className="section-title">Contato</h3>
                <div className="contact-list">
                  {data?.telephones?.whatsapp?.map((phone: string, index: number) => (
                  ...
                  {data?.telephones?.telephone?.map((phone: string, index: number) => (
                    <div key={index} className="contact-item">
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
